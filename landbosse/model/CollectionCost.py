...
class Cable:
    ...
    def __init__(self, cable_specs, addl_specs):
        ...
        self.inductance = cable_specs["Inductance (mH/km)"] * 1e-3
        self.capacitance = cable_specs["Capacitance (nF/km)"] * 1e-9
...
class ArraySystem(CostModule):
    ...
    def calculate_costs(self, calculate_costs_input_dict, calculate_costs_output_dict):
        ...
        cable_cost_usd_per_LF_df = pd.DataFrame(
            [["Materials", self._total_cable_cost, "Collection"]], columns=cost_columns
        )

        # Combine all calculated cost items into the 'collection_cost' dataframe:
        collection_cost = pd.concat(
            (trenching_equipment_rental_cost_df, trenching_labor_cost_df, cable_cost_usd_per_LF_df)
        )
        ...
        mobilization_cost = pd.DataFrame(
            [["Mobilization", calculate_costs_output_dict["mob_cost"], "Collection"]], columns=cost_columns
        )
